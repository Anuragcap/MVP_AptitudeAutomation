...
export default function FileUpload({ onQuestionsGenerated }) {
    ...
    // Simple document processing with AI
    const processDocumentWithAI = async (file) => {
        try {
            // For PDF/PPTX/DOCX, we'll use a simple approach
            // Convert file to base64 and ask AI to extract text content
            const arrayBuffer = await file.arrayBuffer()
            const base64 = btoa(String.fromCharCode(...new Uint8Array(arrayBuffer)))

            // For now, return a placeholder - in production you'd use proper document parsing
            return `Document content from ${file.name}. This is a placeholder for document text extraction.`
        } catch (error) {
            throw new Error('Failed to process document. Please try converting to text format.')
        }
    }
